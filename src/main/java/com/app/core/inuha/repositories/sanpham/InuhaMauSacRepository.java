...
public class InuhaMauSacRepository implements IDAOinterface<InuhaMauSacModel, Integer> {
    ...
    public int insert(InuhaMauSacModel model) throws SQLException {
        ...
        String query = String.format("""
            DELETE FROM %s WHERE trang_thai_xoa = 1 AND id NOT IN (SELECT DISTINCT id_mau_sac FROM SanPhamChiTiet);
    ...
    @Override
    public int update(InuhaMauSacModel model) throws SQLException {
        int result = 0;
        String query = String.format("""
            DELETE FROM %s WHERE trang_thai_xoa = 1 AND id NOT IN (SELECT DISTINCT id_mau_sac FROM SanPhamChiTiet);
            UPDATE %s SET
    ...
    public boolean hasUse(Integer id) throws SQLException {
        String query = "SELECT TOP(1) 1 FROM SanPhamChiTiet WHERE id_mau_sac = ?";
        try {
            return JbdcHelper.value(query, id) != null;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public boolean has(InuhaMauSacModel model) throws SQLException {
        String query = String.format("""
            SELECT TOP(1) 1
            FROM %s
            WHERE
    ...
    @Override
    public Optional<InuhaMauSacModel> getById(Integer id) throws SQLException {
    ...
    @Override
    public List<InuhaMauSacModel> selectAll() throws SQLException {
        List<InuhaMauSacModel> list = new ArrayList<>();
        ResultSet resultSet = null;
        ...
        try {
            ...
            while(resultSet.next()) {
                InuhaMauSacModel model = buildData(resultSet);
                list.add(model);
            }
        } catch(Exception e) {
            e.printStackTrace();
    ...
    @Override
    public List<InuhaMauSacModel> selectPage(FilterRequest request) throws SQLException {
        List<InuhaMauSacModel> list = new ArrayList<>();
        ResultSet resultSet = null;
    ...
    private InuhaMauSacModel buildData(ResultSet resultSet, boolean addSTT) throws SQLException { 
        return InuhaMauSacModel.builder()
