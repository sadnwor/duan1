...
public class InuhaDeGiayRepository implements IDAOinterface<InuhaDeGiayModel, Integer> {
    ...
    public int insert(InuhaDeGiayModel model) throws SQLException {
        ...
        String query = String.format("""
            DELETE FROM %s WHERE trang_thai_xoa = 1 AND id NOT IN (SELECT DISTINCT id_de_giay FROM SanPham);
    ...
    @Override
    public int update(InuhaDeGiayModel model) throws SQLException {
        int result = 0;
        String query = String.format("""
            DELETE FROM %s WHERE trang_thai_xoa = 1 AND id NOT IN (SELECT DISTINCT id_de_giay FROM SanPham);
            UPDATE %s SET
    ...
    public boolean has(InuhaDeGiayModel model) throws SQLException {
        String query = String.format("""
            SELECT TOP(1) 1
            FROM %s
            WHERE
    ...
    @Override
    public Optional<InuhaDeGiayModel> getById(Integer id) throws SQLException {
    ...
    @Override
    public List<InuhaDeGiayModel> selectAll() throws SQLException {
        ...
        try {
            ...
            while(resultSet.next()) {
                InuhaDeGiayModel model = buildData(resultSet);
                list.add(model);
            }
        } catch(Exception e) {
            e.printStackTrace();
    ...
    public Optional<InuhaDeGiayModel> getByName(String name) throws SQLException {
        ResultSet resultSet = null;
        InuhaDeGiayModel model = null;
    ...
    private InuhaDeGiayModel buildData(ResultSet resultSet) throws SQLException { 
        return buildData(resultSet, true);
    }
    
    private InuhaDeGiayModel buildData(ResultSet resultSet, boolean addSTT) throws SQLException { 
        return InuhaDeGiayModel.builder()
