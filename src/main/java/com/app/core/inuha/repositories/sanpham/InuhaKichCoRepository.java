...
public class InuhaKichCoRepository implements IDAOinterface<InuhaKichCoModel, Integer> {
    ...
    public boolean hasUse(Integer id) throws SQLException {
        String query = "SELECT TOP(1) 1 FROM SanPhamChiTiet WHERE id_kich_co = ?";
        try {
            return JbdcHelper.value(query, id) != null;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public boolean has(InuhaKichCoModel model) throws SQLException {
        String query = String.format("""
            SELECT TOP(1) 1
            FROM %s
            WHERE
    ...
    @Override
    public Optional<InuhaKichCoModel> getById(Integer id) throws SQLException {
    ...
    @Override
    public List<InuhaKichCoModel> selectAll() throws SQLException {
        ...
        try {
            ...
            while(resultSet.next()) {
                InuhaKichCoModel model = buildData(resultSet);
                list.add(model);
            }
        } catch(Exception e) {
            e.printStackTrace();
    ...
    @Override
    public List<InuhaKichCoModel> selectPage(FilterRequest request) throws SQLException {
        List<InuhaKichCoModel> list = new ArrayList<>();
        ResultSet resultSet = null;
    ...
    private InuhaKichCoModel buildData(ResultSet resultSet, boolean addSTT) throws SQLException { 
        return InuhaKichCoModel.builder()
