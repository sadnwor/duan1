...
public class InuhaChatLieuRepository implements IDAOinterface<InuhaChatLieuModel, Integer> {
    ...
    public int insert(InuhaChatLieuModel model) throws SQLException {
        ...
        String query = String.format("""
            DELETE FROM %s WHERE trang_thai_xoa = 1 AND id NOT IN (SELECT DISTINCT id_chat_lieu FROM SanPham);
    ...
    @Override
    public int update(InuhaChatLieuModel model) throws SQLException {
        int result = 0;
        String query = String.format("""
            DELETE FROM %s WHERE trang_thai_xoa = 1 AND id NOT IN (SELECT DISTINCT id_chat_lieu FROM SanPham);
            UPDATE %s SET
    ...
    public boolean has(InuhaChatLieuModel model) throws SQLException {
        String query = String.format("""
            SELECT TOP(1) 1
            FROM %s
            WHERE
    ...
    @Override
    public Optional<InuhaChatLieuModel> getById(Integer id) throws SQLException {
    ...
    @Override
    public List<InuhaChatLieuModel> selectAll() throws SQLException {
        List<InuhaChatLieuModel> list = new ArrayList<>();
        ResultSet resultSet = null;
        ...
        try {
            ...
            while(resultSet.next()) {
                InuhaChatLieuModel model = buildData(resultSet);
                list.add(model);
            }
        } catch(Exception e) {
            e.printStackTrace();
    ...
    @Override
    public List<InuhaChatLieuModel> selectPage(FilterRequest request) throws SQLException {
        List<InuhaChatLieuModel> list = new ArrayList<>();
        ResultSet resultSet = null;
    ...
    private InuhaChatLieuModel buildData(ResultSet resultSet, boolean addSTT) throws SQLException { 
        return InuhaChatLieuModel.builder()
