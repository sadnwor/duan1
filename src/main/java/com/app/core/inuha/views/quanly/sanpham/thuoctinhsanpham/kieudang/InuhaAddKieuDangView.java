 ...
 * @author InuHa
 */
public class InuhaAddKieuDangView extends JPanel {
    ...
    /**
     * Creates new form InuhaThemKieuDangView
     */
    public InuhaAddKieuDangView() {
        initComponents();
    ...
    private void handleClickButtoncancel() {
        ModalDialog.closeModal(InuhaListKieuDangView.MODAL_ID_CREATE);
    }
    ...
    private void handleClickButtonSubmit() {
        ...
        if (ten.isEmpty()) { 
            MessageToast.error("Tên kiểu dáng không được bỏ trống");
            return;
        }
        
        if (ten.length() > 250) { 
            MessageToast.error("Tên kiểu dáng không được vượt quá 250 ký tự");
            return;
        }
        
        if (ValidateUtils.isSpecialCharacters(ten)) {
            MessageToast.error("Tên kiểu dáng không được chứa ký tự đặc biệt");
            return;
        }
        ...
        executorService.submit(() -> {
            try {
                kieuDangService.insert(model);
                InuhaAddSanPhamView.getInstance().loadDataKieuDang();
                InuhaListKieuDangView.getInstance().loadDataPage(1);
		InuhaSanPhamView.getInstance().loadDataKieuDang();
                ModalDialog.closeModal(InuhaListKieuDangView.MODAL_ID_CREATE);
                MessageToast.success("Thêm mới kiểu dáng thành công!");
