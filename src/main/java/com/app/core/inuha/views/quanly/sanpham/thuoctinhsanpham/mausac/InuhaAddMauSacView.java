 ...
 * @author InuHa
 */
public class InuhaAddMauSacView extends JPanel {
    ...
    /**
     * Creates new form InuhaThemMauSacView
     */
    public InuhaAddMauSacView() {
        initComponents();
    ...
    private void handleClickButtoncancel() {
        ModalDialog.closeModal(InuhaListMauSacView.MODAL_ID_CREATE);
    }
    ...
    private void handleClickButtonSubmit() {
        ...
        if (ten.isEmpty()) { 
            MessageToast.error("Tên màu sắc không được bỏ trống");
            return;
        }
        
        if (ten.length() > 250) { 
            MessageToast.error("Tên màu sắc không được vượt quá 250 ký tự");
            return;
        }
        
        if (ValidateUtils.isSpecialCharacters(ten)) {
            MessageToast.error("Tên màu sắc không được chứa ký tự đặc biệt");
            return;
        }
